...
#include "quad_renderer_opengles3.2.h"
#include <stdlib.h>
#include <gaven.h>
quad_renderer* create_quad_renderer(renderer_api* renderer,int Quad_Cap){
    ...
    GAVEN_ASSERT(R->Indices,"Couldnt allocate memory for opengl es 3.2 renderer index array");
    R->Indices_Count=0;
...
void draw_quad(void* renderer, int x,int y, int width, int height,float red,float green,float blue,float alpha){
    ...
    if(R->Indices_Count==R->Indices_Cap-5){
        ...
        GAVEN_ASSERT(R->Indices,"Couldnt allocate memory for opengl es 3.2 quad renderer index array");
    }
    if(R->Vertices_Count==R->Vertices_Cap-3){
        R->Vertices_Cap*=2;
        R->Vertices_Data=realloc(R->Vertices_Data,sizeof(quad_vertex_data)*R->Vertices_Cap);
        GAVEN_ASSERT(R->Vertices_Data,"Couldnt allocate memory for opengl es 3.2 quad renderer index array");
    }
